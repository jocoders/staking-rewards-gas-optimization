...
contract StakingRewardsOptimized is Owned, ReentrancyGuard {
  ...
  ) payable Owned(_owner) ReentrancyGuard() {
  ...
  function lastTimeRewardApplicable() public view returns (uint32 _lastTime) {
    assembly {
      ...
      if iszero(lt(_lastTime, _periodFinish)) {
        _lastTime := _periodFinish
      }
    }
  }
  ...
  function setPaused(bool _paused) external onlyOwner {
    ...
    assembly {
      ...
      if iszero(iszero(eq(_pausedSaved, _paused))) {
        return(0, 0)
      }
   ...
   * @param _reward The new duration in seconds.
   */
  function notifyRewardAmount(uint256 reward) external {
    ...
    assembly {
      ...
      switch _condition
      case 1 {
        _rewardRate := div(_rewardRate, _rewardsDuration)
      }
      default {
  ...
  function stake(uint256 amount) external nonReentrant {
    ...
    assembly {
      ...
      if iszero(iszero(lt(_newBal, _senderBal))) {
        revert(0, 0)
      }
      ...
      if iszero(iszero(lt(_newSupply, _supply))) {
        revert(0, 0)
      }
    ...
    _require(_returnSize == 0 ? address(_token).code.length == 0 : _returnValue != 1, SafeERC20FailedOperationSelector);
  ...
  function withdraw(uint256 amount) public nonReentrant {
    ...
    assembly {
      ...
      if iszero(iszero(gt(_newBal, _senderBal))) {
        revert(0, 0)
      }
      ...
      if iszero(iszero(gt(_newSupply, _supply))) {
        revert(0, 0)
      }
      sstore(SLOT_TOTAL_SUPPLY, _newSupply)
  ...
  function _safeTransfer(address token, uint256 amount, uint256 selector) private {
    ...
    _require(_returnSize == 0 ? address(token).code.length == 0 : _returnValue != 1, SafeERC20FailedOperationSelector);
  ...
  function _updateReward(address account) private {
    ...
    assembly {
      ...
      if iszero(iszero(account)) {
        ...
        let _earned := add(
          div(mul(sub(_rewardPerToken, _userRewardPerTokenPaid), _senderBalance), 0xF4240),
          _userRewards
        )
